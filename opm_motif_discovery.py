"""
OPM-based motif discovery for EEG time series.

Order Preserving Matching (OPM) identifies recurrent subsequences
that share the same rank ordering, capturing ordinal temporal patterns
independent of absolute amplitude values.
...
# =========================
#   ORDER PATTERN ENCODING
# =========================
def order_pattern_fast(substring):
    """Convert substring to order-preserving pattern as ranks tuple."""
    sorted_idx = sorted(range(len(substring)), key=lambda i: (substring[i], i))
    rank = [0] * len(substring)
...
class MotifDiscoveryOPM:
    ...
    def find_motifs_single_length(self, sequences, motif_length, min_support=0.9):
        """Find order-preserving motifs of fixed length with support filtering."""
        ...
        for seq_idx, numeric_seq in enumerate(sequences):
            ...
            for i in range(n - motif_length + 1):
                ...
                motif = order_pattern_fast(substring)
...
def remove_substring_motifs(motifs_dict):
    ...
    for m in motifs_sorted:
        ...
        for kept in maximal_motifs:
            for start in range(len(kept) - len(m) + 1):
                if kept[start:start + len(m)] == m:
                    is_substring = True
                    break
            if is_substring:
                break
...
def compare_and_save(control_columns, adhd_columns, motif_finder, output_file, min_support=0.9):
    ...
    print(f"OPM results saved to {output_file}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="OPM-based EEG motif discovery")
    ...
    motif_finder = MotifDiscoveryOPM(min_length=args.min_length, max_length=args.max_length)
